...
if __name__ == "__main__":
    ...
    query.add_argument(
        "number", type=int, default=500, choices=range(1, 1001), help="Batch size for making sub queries to get up to max number of records")
    ...
    query.add_argument("-a", "--align",
                       help="Attempt to get aligning records over multiple sensors", action="store_true")
    query.add_argument("-w", "--window", type=int,
                       help="+/- value in minutes to search for corresponding records", default=5)
    query.add_argument("-m", "--max_records", type=int,
                       help="Max number of records to download, default is total number of records from first query")
    query.add_argument("-i", "--interval-minutes", type=int, default=360,
                       help="Number of minutes to use as an interval when doing sub searches to fetch up to max_records when more than 10000 records exist in the time interval between start-time and end-date. Defaults to 360 minutes (6 hours)")
    ...
    # pull-user-data
    user_data_help = "Download data for the given writer_id"
    user_data = subparser.add_parser(
        "user-data", help=user_data_help, description=user_data_help)
    user_data.add_argument("-f", "--userfile", type=str,
                           help="File containing a users to query. One per line")

    user_data.add_argument("-w", "--writer_id", type=str,
                           help="Download this writer_id or device_id or android_id files into ./downloads")

    user_data.set_defaults(func=wash.user_data)
    ...
    # Example: baseline iterate
    baseline_help = "Query a known question in the baseline survey and iterate results."
    baseline = subparser.add_parser(
        "example-baseline", help=baseline_help, description=baseline_help)
    baseline.set_defaults(func=wash.example_iterate_baseline)
    ...
    # Example: Users with traumatic_brain_injury
    traumatic_brain_injury_help = "Use the survey to find users who reported a head injury."
    traumatic_brain_injury = subparser.add_parser(
        "traumatic_brain_injury", help=traumatic_brain_injury_help, description=traumatic_brain_injury_help)
    traumatic_brain_injury.set_defaults(
        func=wash.traumatic_brain_injury)
